...
namespace SCM.Ver
{
    public partial class frmHistNCProvedor : Form
    {
        public frmHistNCProvedor()
        {
            InitializeComponent();
        }

        private void frmHistNCProvedor_Load(object sender, EventArgs e)
        {
        ...
        void click_buscar()
        {
            try
            {
                uGridFactura.DisplayLayout.ResetBands();
                DataTable dt; 
                dt = NNotasCreditos.buscar_rpt_nc(Convert.ToInt32(this.cmbProveedor.Value), Convert.ToDateTime(this.dtpDesde.Value), Convert.ToDateTime(this.dtpHasta.Value));
