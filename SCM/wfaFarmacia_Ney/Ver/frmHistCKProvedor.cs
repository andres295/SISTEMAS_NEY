...
namespace SCM.Ver
{
    public partial class frmHistCKProvedor : Form
    {
        public frmHistCKProvedor()
        {
            InitializeComponent();
        }

        private void frmHistCKProvedor_Load(object sender, EventArgs e)
        {
        ...
        void click_buscar()
        {
            try
            {
                ...
                DataTable dt;
                dt = NNotasCreditos.buscar_rpt_ck(Convert.ToInt32(this.cmbProveedor.Value), Convert.ToDateTime(this.dtpDesde.Value), Convert.ToDateTime(this.dtpHasta.Value));
