...
namespace SCM.Ver
{
    public partial class frmRptHistorialTipoPago : Form
    {
        public static frmRptHistorialTipoPago me; 
        public frmRptHistorialTipoPago()
        {
            frmRptHistorialTipoPago.me = this;
            InitializeComponent();
        } 
        ...
        private void btnCargarHistorico_Click(object sender, EventArgs e)
        {
            ...
            {
                myReportDocument.Load(System.Windows.Forms.Application.StartupPath + "\\Reportes\\rptHisoricoTipoPagoVenta_Det.rpt");
            }
            else
            {
                myReportDocument.Load(System.Windows.Forms.Application.StartupPath + "\\Reportes\\rptHisoricoTipoPagoVenta.rpt");
            }
            ...
            myReportDocument.SetParameterValue("@Desde", dtpDesde.Value.ToString("yyyy-MM-dd"));
            myReportDocument.SetParameterValue("@Hasta", dtpHasta.Value.ToString("yyyy-MM-dd"));
            myReportDocument.SetParameterValue("@TIPO_PAGO", cbTipo.Value);
        ...
        private void CargarTipoPago()
        {
            try
            {
                DataTable dtTipo = NTipoPago.buscarSinCredito("*");

                var row = dtTipo.NewRow();
                row["ID"] = "0";
                row["Tipo"] = "TODOS";
                dtTipo.Rows.InsertAt(row, 0);

                this.cbTipo.DataSource = dtTipo;
                this.cbTipo.ValueMember = "ID";
                this.cbTipo.DisplayMember = "Tipo";

                this.dtpDesde.Value = DateTime.Now.Date;
                this.dtpDesde.Value = this.dtpDesde.Value.AddDays(-7);
                this.dtpHasta.Value = DateTime.Now.Date;
                this.cbTipo.AutoCompleteMode = Infragistics.Win.AutoCompleteMode.Suggest;
                if (this.cbTipo.Items.Count > 0)
