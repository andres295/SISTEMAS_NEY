...
namespace SCM.CxP
{
    public partial class frmCuentasPorPagar_NC_Productos : Form
    {
        public bool filtrar_producto = false;
        public frmCuentasPorPagar_NC_Productos()
        {
            InitializeComponent();
        }
        ...
        private void frmCuentasPorPagar_NC_Productos_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (frmCuentasPorPagar_NC.me.btnGuardar.Enabled == true)
                {
                    if (frmCuentasPorPagar_NC.me.dgvProductos.Rows.Count == 1)
                        this.Text = this.Text + " | HAY 1 PRODUCTO AGREGADO";
                    else
                        this.Text = this.Text + " | HAY " + frmCuentasPorPagar_NC.me.dgvProductos.Rows.Count + " PRODUCTOS AGREGADOS";
                }
                ...
                {
                    this.dgvProductos.DataSource = NNotasCreditos.buscar_prod("", "");
                }
        ...
        void click_guardar()
        {
            try
            {
                ...
                this.ttMensaje.Hide(this.nudBonificacion);
                ...
                if (this.nudBonificacion.Text == "")
                {
                    ...
                    this.nudBonificacion.Text = this.nudBonificacion.Value.ToString();
                }
                else
                    this.nudBonificacion.Value = Convert.ToDecimal(this.nudBonificacion.Text);
                ...
                else if (Convert.ToInt32(this.nudBonificacion.Value) > 0 && Convert.ToInt32(this.dgvProductos.CurrentRow.Cells[3].Value) == 0)
                {
                    this.ttMensaje.ToolTipTitle = "AVISO";
                    this.ttMensaje.ToolTipIcon = ToolTipIcon.Warning;
                    this.ttMensaje.Show("No hay bonificación", this.nudBonificacion, 0, 26, 3000);
                    this.nudBonificacion.Focus();
                    return;
                }
                else if (Convert.ToInt32(this.nudBonificacion.Value) > Convert.ToInt32(this.dgvProductos.CurrentRow.Cells[3].Value))
                {
                    this.ttMensaje.ToolTipTitle = "ERROR";
                    this.ttMensaje.ToolTipIcon = ToolTipIcon.Error;
                    this.ttMensaje.Show("La bonificación es mayor a la real", this.nudBonificacion, 0, 26, 3000);
                    this.nudBonificacion.Focus();
                    return;
                }
                else if (NNotasCreditos.existe_prod(frmCuentasPorPagar_NC.me.dgvFacturas.CurrentRow.Cells[0].Value.ToString(), frmCuentasPorPagar_NC.me.dgvFacturas.CurrentRow.Cells[1].Value.ToString(), this.dgvProductos.CurrentRow.Cells[0].Value.ToString()) > 0)
                {
                    this.ttMensaje.ToolTipTitle = "ERROR";
                    this.ttMensaje.ToolTipIcon = ToolTipIcon.Error;
                ...
                NNotasCreditos.guardar_prod_temp(frmCuentasPorPagar_NC.me.dgvFacturas.CurrentRow.Cells[0].Value.ToString(), frmCuentasPorPagar_NC.me.dgvFacturas.CurrentRow.Cells[1].Value.ToString(), this.dgvProductos.CurrentRow.Cells[0].Value.ToString(), Convert.ToInt32(this.nudCantidad.Value), Convert.ToInt32(this.nudBonificacion.Value));

                frmCuentasPorPagar_NC.me.dgvProductos.DataSource = NNotasCreditos.cargar_prod(frmCuentasPorPagar_NC.me.dgvFacturas.CurrentRow.Cells[0].Value.ToString());
                frmCuentasPorPagar_NC.me.orden_prod(frmCuentasPorPagar_NC.me.dgvProductos);
                frmCuentasPorPagar_NC.me.cargar_totales();

                frmCuentasPorPagar_NC.me.btnModificarProd.Enabled = true;
                frmCuentasPorPagar_NC.me.btnEliminarProd.Enabled = true;
                frmCuentasPorPagar_NC.me.btnGuardar.Enabled = true;
        ...
        private void tBuscar_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                this.dgvProductos.DataSource = NNotasCreditos.buscar_prod(frmCuentasPorPagar_NC.me.dgvFacturas.CurrentRow.Cells[1].Value.ToString(), this.txtBuscar.Text);
                this.orden();
        ...
        private void cargar_productos_automatico()
        {
            try
            {
                if (frmCuentasPorPagar_NC.me.dgvFacturas.RowCount>0)
                {
                    this.tBuscar.Enabled = false;
