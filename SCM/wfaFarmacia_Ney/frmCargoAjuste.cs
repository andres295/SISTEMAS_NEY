...
namespace SCM
{
    public partial class frmCargoAjuste : Form
    {
        public static frmCargoAjuste me;

        public frmCargoAjuste()
        {
            frmCargoAjuste.me = this;
            InitializeComponent();
        }
        ...
        public void cargar_totales(string fact)
        {
            try
            {
                DataTable dt_datos = NCargoAjuste.obtener_totales(fact);
        ...
        {
            frmCargoAjuste_Acciones frm = new frmCargoAjuste_Acciones();
            frm.Text = "AGREGAR";
        ...
        private void frmCargoAjuste_Load(object sender, EventArgs e)
        {
            ...
            this.dgvFacturas.DataSource = NCargoAjuste.buscar_fact(cGeneral.id_user_actual, "", Convert.ToDateTime(dtpDesde.Value), Convert.ToDateTime(dtpHasta.Value));
            this.orden_fact(this.dgvFacturas);
            this.dgvProductos.DataSource = NCargoAjuste.cargar_prod(cGeneral.id_user_actual, "");

            this.txtRegistros.Text = NCargoAjuste.num_reg().ToString("N0");
        ...
        private void btnModificar_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt_datos = NCargoAjuste.obtener_datos(cGeneral.id_user_actual, Convert.ToInt32(this.dgvFacturas.CurrentRow.Cells[0].Value));
        ...
        private void tBuscar_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                this.dgvFacturas.DataSource = NCargoAjuste.buscar_fact(cGeneral.id_user_actual, this.txtBuscar.Text, Convert.ToDateTime(dtpDesde.Value), Convert.ToDateTime(dtpHasta.Value));
                this.orden_fact(this.dgvFacturas);
        ...
        private void btnAgregarProd_Click(object sender, EventArgs e)
        {
            if (this.dgvFacturas.Rows.Count > 0)
            {
                frmCargoAjuste_Productos frm = new frmCargoAjuste_Productos();
                frm.ShowDialog();
            }
            else
            {
        ...
        void click_eliminar_CJ()
        {
            try
            {
                DialogResult resul = MessageBox.Show("Está seguro(a) de eliminar éste Cargo Ajuste.", cGeneral.name_system, MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                ...
                DataTable dtProducto = NCargoCompra.valida_disponiblidad_producto("CARGO AJUSTE", this.dgvFacturas.CurrentRow.Cells[0].Value.ToString());

                if (dtProducto.Rows.Count <= 0)
                {
                    NCargoAjuste.eliminar_CJ(Convert.ToInt32(this.dgvFacturas.CurrentRow.Cells[0].Value));
        ...
        private void tCargarProd_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.dgvFacturas.Rows.Count > 0)
                {
                    this.dgvProductos.DataSource = NCargoAjuste.cargar_prod(cGeneral.id_user_actual, this.dgvFacturas.CurrentRow.Cells[0].Value.ToString());
                    cargar_totales(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString());

                    if (NCargoAjuste.verificar_cant_prod(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString()) == 1)
                    {
                    ...
                    else if (NCargoAjuste.verificar_cant_prod(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString()) == 2)
                    {
        ...
        void click_eliminar_prod()
        {
            try
            {
                ...
                NCargoAjuste.eliminar_prod(Convert.ToInt32(this.dgvFacturas.CurrentRow.Cells[0].Value), this.dgvProductos.CurrentRow.Cells["Id"].Value.ToString());
                this.dgvProductos.Rows.Remove(this.dgvProductos.CurrentRow);

                this.dgvFacturas.CurrentRow.Cells[2].Value = NCargoAjuste.actualizar_nombre_prov(cGeneral.id_user_actual, Convert.ToInt32(this.dgvFacturas.CurrentRow.Cells[0].Value));
                DataTable dt_totales = NCargoAjuste.obtener_totales(frmCargoAjuste.me.dgvFacturas.CurrentRow.Cells[0].Value.ToString());
        ...
        private void btnModificarProd_Click(object sender, EventArgs e)
        {
            if (this.dgvProductos.Rows.Count > 0)
            {
                frmCargoAjuste_Cantidad frm = new frmCargoAjuste_Cantidad();
        ...
        public void print_cargo_ajuste()
        {
            try
            {
                if (this.dgvProductos.Rows.Count > 0)
                {
                    DialogResult resul = MessageBox.Show("Desea imprimir el documento de Cargo Ajuste.", cGeneral.name_system, MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    ...
                    Reportes.frmViewRpt frmReporte = new Reportes.frmViewRpt("cargo_ajuste_inv");
        ...
        void click_guardar()
        {
            try
            {
                ...
                NCargoAjuste.guardar(cGeneral.id_user_actual, Convert.ToInt32(this.dgvFacturas.CurrentRow.Cells[0].Value));
                this.dgvFacturas.CurrentRow.Cells[2].Value = NCargoAjuste.actualizar_nombre_prov(cGeneral.id_user_actual, Convert.ToInt32(this.dgvFacturas.CurrentRow.Cells[0].Value));
        ...
        {
            Reportes.frmViewRpt frmReporte = new Reportes.frmViewRpt("cargo_ajuste_inventario_rpt");
        ...
        private void btnSubir_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvFacturas.Rows.Count > 0)
                {
                    gvProcesando.Visible = true;
                    OpenFileDialog openFileDialog1 = new OpenFileDialog();
                    openFileDialog1.Filter = "Excel Files(.xls)|*.xls| Excel Files(.xlsx)| *.xlsx | Excel Files(*.xlsm) | *.xlsm";
                    openFileDialog1.Title = "SELECCIONE EL ARCHIVO A SUBIR";

                    if (openFileDialog1.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        DialogResult resul = MessageBox.Show("Esta seguro que desea subir estos productos para cargarlos a este Cargo Ajuste?", cGeneral.name_system, MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                        ...
                        //string conexion = "Provider = Microsoft.Jet.OleDb.4.0;Data Source = " + txtArchivoSubir.Text + " ; Extended Properties = \"Excel 8.0;HDR = Yes\"";
                        string conexion = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + openFileDialog1.FileName.ToString() + ";Extended Properties = \"Excel 8.0;HDR = Yes\"";
                        ...
                        consulta = new OleDbCommand("select * from [Datos$]", conector);

                        OleDbDataAdapter adaptaror = new OleDbDataAdapter();
                        adaptaror.SelectCommand = consulta;
                        ...
                        try
                        {
                            if (validaDatosCargExcel(ds))
                            {
                                if (NCargoAjuste.guardar_prod_archivo_excel(cGeneral.id_user_actual, dgvFacturas.CurrentRow.Cells[0].Value.ToString(), ds))
                                {
                                    dgvProductos.DataSource = NCargoAjuste.cargar_prod(cGeneral.id_user_actual, dgvFacturas.CurrentRow.Cells[0].Value.ToString());
                                    cargar_totales(dgvFacturas.CurrentRow.Cells[0].Value.ToString());
                                    gvProcesando.Visible = false;
                                    btnAgregarProd.Enabled = true;
                                    btnModificarProd.Enabled = true;
                                    btnEliminarProd.Enabled = true;
                                    bntReImprimir.Enabled = true;
                                    btnGuardar.Enabled = true;
                                    MessageBox.Show("Arhivo " + openFileDialog1.FileName.ToString() + " procesado con éxito.", "Archivo procesado", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                }
                                else
                                {
                                    gvProcesando.Visible = false;
                                    MessageBox.Show("Archivo no se pudo procesar", "No se peude procesar el archivo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                }
                            }
                            else
                            {
                        ...
                        {
                            gvProcesando.Visible = false;
                            MessageBox.Show("No se peude procesar el archivo error:" + ex.Message.ToString(), "Archivo no se pudo procesar", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    ...
                    {
                        gvProcesando.Visible = false;
                        MessageBox.Show("Tiene que selecionar un archivo", "Seleccionar archivo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
                else
                {
                    gvProcesando.Visible = false;
                    MessageBox.Show("No se ha creado un registro de cargo ajuste, primero debe de guardar el cargo ajuste para luego agregar sus productos", "No se ha guardado el gargo ajuste", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
                gvProcesando.Visible = false;
                MessageBox.Show(ex.Message.ToString(), "Error al procesar el archivo excel formato .XSL", MessageBoxButtons.OK, MessageBoxIcon.Error);
                cGeneral.error(ex);
            }
        }
        private bool validaDatosCargExcel(DataSet ds)
        {
            bool resul = true;
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        codProducto = ds.Tables[0].Rows[i]["ID"].ToString();
                        try
                        {
                            decimal.Parse(ds.Tables[0].Rows[i]["Disponible"].ToString());
                        }
                        catch (Exception ex)
                        {
                            resul = false;
                            gvProcesando.Visible = false;
                            MessageBox.Show("Error al validar el campo Disponible, este no tiene datos correcto. ID: " + codProducto + " Cantidad: " + ds.Tables[0].Rows[i]["Disponible"].ToString() + " no valida ", "Error al validar disponibilidad", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            break;
                        }
                    }
                }
            ...
            {
                resul = false;
                MessageBox.Show("Error al validar el campo Disponible: " + ex.Message.ToString(), "Error al validar disponibilidad", MessageBoxButtons.OK, MessageBoxIcon.Error);
