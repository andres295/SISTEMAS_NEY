...
namespace SCM.CxC
{
    public partial class frmHistNCCliente : Form
    {
        public frmHistNCCliente()
        {
            InitializeComponent();
        }

        private void frmHistNCCliente_Load(object sender, EventArgs e)
        {
        ...
        void click_buscar()
        {
            try
            {
                ...
                DataTable dt; 
                dt = NNotasCreditosCXC.buscar_rpt_nc(Convert.ToInt32(this.cmbCliente.Value), Convert.ToDateTime(this.dtpDesde.Value), Convert.ToDateTime(this.dtpHasta.Value));
