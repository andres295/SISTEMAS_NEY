...
namespace SCM.Administracion
{
    public partial class frmGastos : Form
    {
        ...
        private void cargarCatGastos()
        {
            ...
            try
            {
                bsGastos.DataSource = gvGastos.DataSource = NAdministracion.Cargar_Cat_Gastos();
        ...
        private void tlEliminarDescuento_Click(object sender, EventArgs e)
        {
            try
            {
                if (gvGastos.Rows.Count>0)
                {
                    DialogResult resul = MessageBox.Show("¿Desea eliminar el Catálogo de gastos?: " + this.gvGastos.Selected.Rows[0].Cells[1].Value.ToString() + "?", "Confirmar eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                    if (resul == System.Windows.Forms.DialogResult.No)
                    {
                        return;
                    }
                    if (NAdministracion.Eliminar_Activar_Cat_Gastos(this.gvGastos.Selected.Rows[0].Cells[0].Value.ToString(), false))
                    {
                        MessageBox.Show("Catálogo de gasto eliminado con éxito. ", "Catálogo de gastos", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void btnGuardar_Click_1(object sender, EventArgs e)
        {
            ...
            if (new_egreso)
            {
                bool result =  NAdministracion.Agregar_Cat_Gastos(txtDescripcion.Text.Trim().ToUpper(), cGeneral.name_user);
                if (result)
                {
                    MessageBox.Show("Catálogo de gastos agregado con éxito!.", "Gastos de conceptos", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    cargarCatGastos();
                    utfrmGastos.Visible = false;
                } 
            }
            else
            {
                if (id_gasto > 0)
                {
                    bool result = NAdministracion.Actualizar_Cat_Gastos(id_gasto.ToString().ToUpper(), txtDescripcion.Text.Trim());
                    if (result)
                    {
                        MessageBox.Show("Catálogo de gastos actualizado con éxito!.", "Gastos de conceptos", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        cargarCatGastos();
                        utfrmGastos.Visible = false;
                    }
                }
                ...
                {
                    MessageBox.Show("No ha selecionado un registro para editar", "Aviso de registro", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        }
