...
namespace SCM.CentroMedico
{
    ...
    {
        public static frmEspecialista me;
        public bool userExterno = false;
        public frmEspecialista()
        {
            frmEspecialista.me = this;
            InitializeComponent();
        }
        ...
        private void frmEmpleados_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.dgvEmpleados.DataSource = NEspecialista.buscar("");
                this.empleados_sort(2);
                this.txtRegistros.Text = NEspecialista.num_reg().ToString("N0");
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                frm.btnComposicionProducto.Enabled = false;
                frm.btnRefrescarPresentacion.Enabled = false;
                frm.ShowDialog();
            }
            catch (Exception ex) { cGeneral.error(ex); }
        }
        ...
        private void load_edit()
        {
            try
            {
                ...
                dt_datos = NEspecialista.obtener_datos(Convert.ToInt32(this.dgvEmpleados.CurrentRow.Cells[0].Value));

                CentroMedico.frmEspecialista_Acciones frm = new   CentroMedico  .frmEspecialista_Acciones(dt_datos.Rows[0].ItemArray[1].ToString());
                frm.Text = "MODIFICAR LOS DATOS DEL ESPECIALISTA";
                frm.accion = true;

                frm.mtxtCedula.Mask = cGeneral.formato_cedula;
                frm.mtxtTelefono.Mask = cGeneral.formato_telefono;

                frm.idEspecialista = Convert.ToInt32(this.dgvEmpleados.CurrentRow.Cells[0].Value);
                ...
                frm.cbEstado.Checked = Convert.ToBoolean(dt_datos.Rows[0].ItemArray[8].ToString());
        ...
        private void tBuscar_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                this.dgvEmpleados.DataSource = NEspecialista.buscar(this.txtBuscar.Text);
        ...
        void click_eliminar()
        {
            try
            {
                DialogResult resul = MessageBox.Show("Estás seguro(a) de eliminar el especialista.", cGeneral.name_system, MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                if (resul == System.Windows.Forms.DialogResult.Yes)
                {
                    if (NEspecialista.eliminar(Convert.ToInt32(this.dgvEmpleados.CurrentRow.Cells[0].Value)))
                    {
        ...
        private void txtBuscar_MouseEnter(object sender, EventArgs e)
        {
            try
            {
                ...
                this.ttMensaje.Show("Busca el # cédula, nombres, apellidos, género, teléfono y dirección del especialista", this.txtBuscar, 0, 26);
            }
            catch (Exception ex) { cGeneral.error(ex); }
        }
