...
{
    public partial class frmDescuentoLinea : Form
    { 
        public bool accion;
        ...
        public static frmDescuentoLinea me;
        public frmDescuentoLinea()
        {
            frmDescuentoLinea.me = this;
            InitializeComponent();
        } 
        ...
        private void cargarDescuentos()
        {
            ...
            try
            {
                bindingSource1.DataSource = gvDescuentoLinea.DataSource = NDescuentos.Obtener_Descuento_Linea();
                nvDescuentos.BindingSource = bindingSource1;
                gvDescuentoLinea.DataSource = bindingSource1;
            }
            catch (Exception)  {  }
        }
        ...
        private void gvDescuentoLinea_InitializeLayout(object sender, InitializeLayoutEventArgs e)
        {
            try
            {
        ...
        private void tlExpotarExcel_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.gvDescuentoLinea.Rows.Count > 0)
                {
                    DateTime start;
                    start = DateTime.Now;
                    TimeSpan timespan;
                    ...
                    this.ultraGridExcelExporter1.Export(this.gvDescuentoLinea, sfilePath);
        ...
        {
            if (gvDescuentoLinea.Rows.Count>0)
            {
                try
                {
                   
                    Descuentos.frmAgregarDescuentoLinea frm = new Descuentos.frmAgregarDescuentoLinea();
                    frm.lblID.Text = this.gvDescuentoLinea.Selected.Rows[0].Cells[0].Value.ToString();
                    frm.txtDescripcion.Text = this.gvDescuentoLinea.Selected.Rows[0].Cells[1].Value.ToString();
                    frm.nudPorcentaje.Value =   decimal.Parse(this.gvDescuentoLinea.Selected.Rows[0].Cells[2].Value.ToString());
                    frm.dtpFechaInicio.Value = DateTime.Parse(this.gvDescuentoLinea.Selected.Rows[0].Cells[3].Value.ToString());
                    frm.dtpFechaFin.Value = DateTime.Parse(this.gvDescuentoLinea.Selected.Rows[0].Cells[4].Value.ToString());
                    frm.cbEstado.Checked = bool.Parse(this.gvDescuentoLinea.Selected.Rows[0].Cells[6].Value.ToString());
                    frm.ShowDialog();
                }
        ...
        private void gvDescuentoLinea_DoubleClickRow(object sender, DoubleClickRowEventArgs e)
        {
            dbNuevoDescuento_Click(null, null);
        ...
        private void tlEliminarDescuento_Click(object sender, EventArgs e)
        {
            try
            {
                if (gvDescuentoLinea.Rows.Count>0)
                {
                    DialogResult resul = MessageBox.Show("¿Desea eliminar el descuento de linea: " + this.gvDescuentoLinea.Selected.Rows[0].Cells[1].Value.ToString() + "?", "Confirmar aprobación de Precio Especial", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                    if (resul == System.Windows.Forms.DialogResult.No)
                    {
                        return;
                    }
                    if (NDescuentos.eliminar_descuento_linea(int.Parse(this.gvDescuentoLinea.Selected.Rows[0].Cells[0].Value.ToString())) >0)
                    {
                        MessageBox.Show("Descuento de linea eliminado con éxito. ", "Descuento de LINEA", MessageBoxButtons.OK, MessageBoxIcon.Information);
