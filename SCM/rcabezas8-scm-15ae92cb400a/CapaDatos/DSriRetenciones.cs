...
namespace CapaDatos
{
    public class DSriRetenciones
    {
        DConexion con;
        string texto = "Hubo un problema en la capa DSriRetenciones";
        ...
        { 
            try
            {
                ...
                {
                    SqlCommand cmd;
                    cmd = new SqlCommand("INSERT INTO TmpRetencionesSRIDet (ImpuestoRetener, ValorImpuesto, BaseImponible , PorcRetencion, ValorRetenido , IdUsuario) VALUES('" + impuestoRetener + "','" + idValorImpuesto + "','" + baseimponible.ToString().Replace(",", ".") + "','" + porcRetencion.ToString().Replace(",", ".") + "','" + valorretenido.ToString().Replace(",", ".") + "','" + idUsuario + "')", con.conexion);
        ...
        public DataTable Obtener_Det_TMP_Retenciones(int idUsuario)
        {
            try
            {
                ...
                if (con.conectar())
                {
                    SqlDataAdapter da = new SqlDataAdapter("SELECT det.Id,  case when  ImpuestoRetener = '1' THEN 'RENTA' when   ImpuestoRetener = '2' then 'IVA'  when ImpuestoRetener = '3' THEN 'ISD' else 'OTROS'  end as ImpuestoRetener,case when  ImpuestoRetener = '1' THEN cat.Descripcion when   ImpuestoRetener = '2' then porcIva.Descripcion  when ImpuestoRetener = '3' THEN 'ISD' else 'N/A'  end as ValorImpuesto,det.PorcRetencion,BaseImponible, ValorRetenido  FROM TmpRetencionesSRIDet det WITH (NOLOCK)  LEFT JOIN dbo.TBLCatalogosGenerales cat on valorimpuesto = cat.id LEFT JOIN dbo.PorcentajeIva porcIva on valorimpuesto = porcIva.id Where idUsuario = '" + idUsuario + "'  ORDER BY Id", con.conexion);
                    da.SelectCommand.CommandType = CommandType.Text;
                    da.Fill(dt); 
                    con.desconectar();
                } 
        ...
        public bool Delete_Det_TMP_Retenciones(int idUsuario)
        {
            try
            {
                ...
                if (con.conectar())
                {
                    SqlCommand cmd = new SqlCommand("DELETE TmpRetencionesSRIDet WHERE idUsuario = '" + idUsuario + "'", con.conexion);
                    cmd.CommandType = CommandType.Text;
        ...
        public bool Delete_Det_TMP_Retenciones_by_id(int id)
        {
            try
            {
                ...
                if (con.conectar())
                {
                    SqlCommand cmd = new SqlCommand("DELETE TmpRetencionesSRIDet WHERE Id = '" + id + "'", con.conexion);
                    cmd.CommandType = CommandType.Text;
        ...
        public DataTable Agregar_Retencion(DateTime Fecha, int IdProveedor, DateTime FechaEmision,string TipoDocumento,string NumDocumento,DateTime PeriodoFiscal,int IdUsuarioRegistro)
        {
            try
            {
                DataTable dt = new DataTable();
                ...
                if (con.conectar())
                {
                    ...
                    SqlDataAdapter da;
                    da = new SqlDataAdapter("EXEC SP_Incluir_Retenciones_SRI '" + Fecha.ToShortDateString() + "','" + IdProveedor + "','" + FechaEmision.ToShortDateString() + "','" + TipoDocumento + "','" + NumDocumento + "','" + PeriodoFiscal.ToShortDateString() + "','" + IdUsuarioRegistro + "'", con.conexion);
                    da.SelectCommand.CommandType = CommandType.Text; 
                    da.Fill(dt); 

                    /*Se actualiza el estado de la factura*/
                    SqlCommand cmd;
                    cmd = new SqlCommand("exec SP_Cancelar_Factura_CXP '" + NumDocumento + "'", con.conexion);
                    cmd.CommandType = CommandType.Text;
                    cmd.ExecuteNonQuery();
