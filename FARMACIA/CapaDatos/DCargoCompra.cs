        ...
        public DataTable obtener_totales_fila(string fact, string id_prod, string idItemProducto)
        {
            try
            {
                ...
                if (con.conectar())
                {
                    ...
                    {
                        SqlDataAdapter da = new SqlDataAdapter("SELECT Subtotal, IVA, Descuento, TotalPagar, PUR FROM CargoCompra_Productos_Temp WHERE Id_Factura = '" + fact + "' AND Id_Producto = '" + id_prod + "' and  Id = '" + idItemProducto + "'", con.conexion);
                        da.SelectCommand.CommandType = CommandType.Text;
                        da.Fill(dt);
                        con.desconectar();
                    } 
