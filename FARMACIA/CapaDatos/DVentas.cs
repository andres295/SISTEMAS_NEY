    ...
    public class DVentas
    {
        ...
        public DataTable pagar(string id_cliente,long id_venta, decimal monto_efect, string cheque, DateTime fecha_cobro, int id_banco, decimal cheque_monto, string img_cheque, string tranf, decimal monto_tranf, string img_tranf, decimal monto_TC, decimal monto_TD, string Adquiriente, decimal tc_interes,string id_tipo_venta)
        {
            ...
            try
            {
                ...
                { 
                    /*AHORA*/ 
                    SqlDataAdapter da;
                    da = new SqlDataAdapter("EXEC sp_Create_Venta '" + id_venta + "','" + id_cliente + "','" + id_tipo_venta + "',0,'" + monto_efect.ToString().Replace(",", ".") + "'", con.conexion);
        ...
        public void ActualizaClienteVenta(long id_venta, string id_cliente)
        {
            try
            {
                ...
                if (con.conectar())
                {
                    SqlCommand cmd;
                    /*Actualizamos el cliente de la venta*/
                    cmd = new SqlCommand("Update Ventas Set Id_Cliente = '" + id_cliente + "' Where Id = '" + id_venta + "'", con.conexion);
                    cmd.CommandType = CommandType.Text;
                    cmd.ExecuteNonQuery();
                    con.desconectar();
                }
        ...
        public decimal obtener_monto_venta_cierre(int id_usuario)
        {
            try
            {
                ...
                if (con.conectar())
                {
                    try
                    {
                        SqlCommand cmd = new SqlCommand("SELECT sum(vp.Total_Pagar) Total_Venta FROM  [dbo].[Ventas] v INNER JOIN dbo.Ventas_Productos vp on v.Id = VP.Id_Venta where  v.Id_Usuario = '" + id_usuario + "'  and  cast(v.FechaHora_Registro as date) = cast(getdate() as date)", con.conexion);
                        cmd.CommandType = CommandType.Text;
        ...
        public bool eliminar_detalle_multi_pago_by_usuario(int id_usuario)
        {
            try
            {
                ...
                if (con.conectar())
                {
                    SqlCommand cmd = new SqlCommand("DELETE TmpVentas_Pago WHERE  Id_Venta IN (select  Id from dbo.Ventas   where  FechaHora_Registro between getdate()-7 and getdate()+1 and  Id_Usuario = '" + id_usuario + "') ", con.conexion);
                    cmd.CommandType = CommandType.Text;
        ...
        public DataTable pagar_multi_pago(string id_cliente,long id_venta, decimal monto_efect, string cheque, DateTime fecha_cobro, int id_banco, decimal cheque_monto, string img_cheque, string tranf, decimal monto_tranf, string img_tranf, decimal monto_TC, decimal monto_TD, string Adquiriente, decimal tc_interes, string id_tipo_venta)
        {
            ...
            try
            {
                ...
                { 
                    /*LO NUEVO*/ 
                    SqlDataAdapter da;
                    da = new SqlDataAdapter("EXEC sp_Create_Venta '" + id_venta + "','" + id_cliente + "','" + id_tipo_venta + "',1,'" + monto_efect.ToString().Replace(",", ".") + "'", con.conexion);
                    da.SelectCommand.CommandType = CommandType.Text;
                    da.Fill(dt); 
                    con.desconectar(); 
                }
        ...
        public DataTable Obtener_Producto_Venta(long id_venta)
        {
            try
            {
                ...
                if (con.conectar())
                {
                    SqlDataAdapter da = new SqlDataAdapter("SELECT   Id_Venta,	Id_Producto,	Producto,	Presentacion,	Laboratorio,	Vendio, ROUND(	PVP,2)PVP	,ROUND(Subtotal,2) Subtotal,	ROUND(IVA,2)IVA	,ROUND(Descuento,2) Descuento	,ROUND(Total_Pagar,2)Total_Pagar,	Ganancia,	Contiene,	Disponible,	Parcial,	Lote,	Vencimiento_Texto,	Subtotal_DP,	Subtotal_CP,	Id_Combo,	Id_Precio_Especial   FROM Ventas_Productos where Id_Venta = '" + id_venta + "'", con.conexion);
                    da.SelectCommand.CommandType = CommandType.Text;
        ...
        public string ObtenerNumeroConsecutivoSR(int IdVenta)
        {
            try
            {
                ...
                {
                    DataTable dt = new DataTable();
                    SqlDataAdapter da = new SqlDataAdapter("SELECT isnull(max(NumeroComprobante),0)+1 NumeroComprobante from Ventas", con.conexion);
                    da.SelectCommand.CommandType = CommandType.Text;
                    da.Fill(dt);
                    if (dt.Rows.Count > 0)
                    {
                        result = int.Parse(dt.Rows[0]["NumeroComprobante"].ToString());
                    }

                    SqlCommand cmd2;
                    /*Actualizamos el cliente de la venta*/
                    cmd2 = new SqlCommand("Update Ventas Set NumeroComprobante = " + result + " Where Id = '" + IdVenta + "'", con.conexion);
                    cmd2.CommandType = CommandType.Text;
                    cmd2.ExecuteNonQuery();
