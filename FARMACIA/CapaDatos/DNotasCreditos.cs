...
namespace CapaDatos
{
    public class DNotasCreditos
    {
        ...
        public void guardar_nc(string nc, string fact, int id_prov, string prov, DateTime fecha_emision, string matriz, string ruc, string dir, string tel, string obser)
        {
            try
            {
                con = new DConexion();
                ...
                if (con.conectar())
                {
                    SqlCommand cmd = new SqlCommand("INSERT INTO NotasCreditos VALUES('" + nc + "', '" + fact + "', " + id_prov + ", '" + prov + "', '" + fecha_emision.ToShortDateString() + "', '" + matriz + "', '" + ruc + "', '" + dir + "', '" + tel + "', '" + obser + "', GETDATE())", con.conexion);
                    cmd.CommandType = CommandType.Text;
        ...
        public void actualizar_stock(string nc, string fact, string id, bool accion)
        {
            try
            {
                ...
                if (con.conectar())
                {
                    ...
                    cmd = new SqlCommand("SELECT SUM( IIF(Contiene = 0, (Cantidad + Bonificacion), (Cantidad * Contiene) + (Bonificacion * Contiene))) AS Cantidades FROM NotasCreditos_Productos WHERE Id_NC = '" + nc + "' AND Id_Producto = '" + id + "'", con.conexion);
                    cmd.CommandType = CommandType.Text;
        ...
        public void deducir_nc(string nc, string fact)
        {
            try
            {
                ...
                if (con.conectar())
                {
                    ...
                    cmd = new SqlCommand("SELECT SUM((Subtotal + IVA) - Descuento) AS Totales FROM NotasCreditos_Productos WHERE Id_NC = '" + nc + "'", con.conexion);
                    cmd.CommandType = CommandType.Text;
        ...
        public void guardar(string nc, string fact)
        {
            try
            {
                ...
                if (con.conectar())
                {
                    ...
                    cmd = new SqlCommand("UPDATE FacturasPorPagar SET Monto_NC = " + captar_monto.ToString().Replace(",", ".") + ", Monto_TotalPagar = (Monto_Fact - Monto_NC) WHERE Factura = '" + fact + "'", con.conexion);
                    cmd.CommandType = CommandType.Text;
        ...
        public void guardar_prod_temp(string nc, string fact, string id, int cant, int bonif)
        {
            try
            {
                ...
                if (con.conectar())
                {
                    ...
                    cmd = new SqlCommand("SELECT CargoCompra_Productos.Producto, CargoCompra_Productos.PVF, CargoCompra.IVA, CargoCompra.Desc_Porc, CargoCompra_Productos.Contiene FROM CargoCompra_Productos INNER JOIN CargoCompra ON CargoCompra.Factura = CargoCompra_Productos.Id_Factura WHERE CargoCompra_Productos.Id_Factura = '" + fact + "' AND CargoCompra_Productos.Id_Producto = '" + id + "'", con.conexion);
                    cmd.CommandType = CommandType.Text;

     
                    try
                    {
                        da = new SqlDataAdapter("SELECT ISNULL(IVA,0) IVA,ISNULL(Descuento,0) DESCUENTO  FROM farmacia.dbo.CargoCompra_Productos WHERE Id_Factura = '" + fact + "' AND  Id_Producto = '" + id + "'", con.conexion);
                        da.Fill(dt);
