...
{
    public partial class frmDescargoAjuste_Cantidad : Form
    {
        public int idItemProducto = 0;
        public frmDescargoAjuste_Cantidad()
        {
            InitializeComponent();
        }
        ...
        private void frmDescargoAjuste_Cantidad_Load(object sender, EventArgs e)
        {
            try
            {
                DataTable dt_datos = NDescargoAjuste.obtener_enteros_fracciones(Convert.ToInt32(frmDescargoAjuste.me.dgvFacturas.CurrentRow.Cells[0].Value), frmDescargoAjuste.me.dgvProductos.CurrentRow.Cells[0].Value.ToString());

                if (NProductos.obtener_cont(frmDescargoAjuste.me.dgvProductos.CurrentRow.Cells[0].Value.ToString()) == 0)
                {
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int captar_cont = NProductos.obtener_cont(frmDescargoAjuste.me.dgvProductos.CurrentRow.Cells[0].Value.ToString());
                try
                {
                    disponibilida = NDescargoAjuste.valida_disponibilidad_stock(frmDescargoAjuste.me.dgvProductos.CurrentRow.Cells[0].Value.ToString());
                }
                catch (Exception) { }
                if (disponibilida <= 0)
                {
                ...
                NDescargoAjuste.modificar_cantidad(Convert.ToInt32(frmDescargoAjuste.me.dgvFacturas.CurrentRow.Cells[0].Value), frmDescargoAjuste.me.dgvProductos.CurrentRow.Cells[0].Value.ToString(), captar_cant, dtpVencimiento.Value, txtLote.Text, idItemProducto.ToString());

                if (this.nudCantidad1.Enabled == true)
                {
                    frmDescargoAjuste.me.dgvProductos.CurrentRow.Cells[3].Value = this.nudCantidad1.Value;
                    frmDescargoAjuste.me.dgvProductos.CurrentRow.Cells[5].Value = NDescargoAjuste.obtener_subtotal(Convert.ToInt32(frmDescargoAjuste.me.dgvFacturas.CurrentRow.Cells[0].Value), frmDescargoAjuste.me.dgvProductos.CurrentRow.Cells[0].Value.ToString());
                }
                else
                {
                    frmDescargoAjuste.me.dgvProductos.CurrentRow.Cells[3].Value = this.nudCantidad2.Value + "F" + this.nudFracciones.Value;
                    frmDescargoAjuste.me.dgvProductos.CurrentRow.Cells[5].Value = NDescargoAjuste.obtener_subtotal(Convert.ToInt32(frmDescargoAjuste.me.dgvFacturas.CurrentRow.Cells[0].Value), frmDescargoAjuste.me.dgvProductos.CurrentRow.Cells[0].Value.ToString());
                }
                frmDescargoAjuste.me.dgvProductos.CurrentRow.Cells[6].Value = dtpVencimiento.Value.ToShortDateString();
                frmDescargoAjuste.me.dgvProductos.CurrentRow.Cells[7].Value = txtLote.Text.Trim();
