...
namespace wfaFarmacia_Ney.MDI
{
    public partial class frmPrincipal : Form
    {
        ...
        private void frmPrincipal_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                DialogResult resul = MessageBox.Show("Desea salirse del Sistema PHARMASYS 1.8", cGeneral.name_system, MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    ...
                    { 
                        foreach (ToolStripMenuItem MI in menuStrip1.Items)
                        {
                            ...
                            { 
                                ...
                                {
                                    if (Convert.ToString(dropDownItem.Name) == dt.Rows[i]["Num_Ventana"].ToString())
                                    {
                                        activeMenu = true;
                ...
                else
                {
                    ...
                    if (cGeneral.tipo_usuario == "ADMINISTRADOR")
                    {
                        foreach (ToolStripMenuItem MI in menuStrip1.Items)
                        {
                            ...
                            foreach (ToolStripMenuItem dropDownItem in MI.DropDownItems)
                            { 
                                if (dropDownItem.Name == "pARAMETROSToolStripMenuItem")
                                {
                                    activeMenu = true;
                                    dropDownItem.Enabled = true;
                                } 
