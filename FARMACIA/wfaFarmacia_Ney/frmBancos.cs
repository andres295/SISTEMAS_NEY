...
namespace wfaFarmacia_Ney
{
    public partial class frmBancos : Form
    {
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            try
            {
                frmBancos_Acciones frm = new frmBancos_Acciones();
                frm.Text = "AGREGAR BANCO";
                frm.accion = false;
        ...
        private void btnModificar_Click(object sender, EventArgs e)
        {
            ...
            {
                frmBancos_Acciones frm = new frmBancos_Acciones();
                frm.Text = "MODIFICAR LOS DATOS DEL BANCO";
                frm.accion = true;

                DataTable dt_datos = new DataTable();
                dt_datos = NBancos.obtener_datos(Convert.ToInt32(this.dgvBancos.CurrentRow.Cells[0].Value));

                frm.mtxtTelefono.Mask = cGeneral.formato_telefono;

                frm.txtBanco.Text = dt_datos.Rows[0].ItemArray[1].ToString();
                frm.mtxtTelefono.Text = dt_datos.Rows[0].ItemArray[2].ToString();
                frm.txtCorreo.Text = dt_datos.Rows[0].ItemArray[3].ToString();
                frm.txtDireccion.Text = dt_datos.Rows[0].ItemArray[4].ToString();
        ...
        private void frmBancos_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.dgvBancos.DataSource = NBancos.buscar("");
                this.bancos_sort(1);
                this.txtRegistros.Text = NBancos.num_reg().ToString("N0");
            ...
            try
            {
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    try
                    {
                        ...
                        this.dgvBancos.DataSource = NBancos.buscar(this.txtBuscar.Text);
                        this.bancos_sort(1);
        ...
        void click_eliminar()
        {
            try
            {
                ...
                {
                    if (NBancos.eliminar(Convert.ToInt32(this.dgvBancos.CurrentRow.Cells[0].Value)))
                    {
