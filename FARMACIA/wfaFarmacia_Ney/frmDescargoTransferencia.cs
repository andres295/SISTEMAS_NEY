...
{
    public partial class frmDescargoTransferencia : Form
    { 
        public static frmDescargoTransferencia me;

        public frmDescargoTransferencia()
        {
            frmDescargoTransferencia.me = this;
            InitializeComponent();
        }
        ...
        public void cargar_totales(string fact)
        {
            try
            {
                DataTable dt_datos = NDescargoTransferencia.obtener_totales(fact);
        ...
        {
            frmDescargoTransferencia_Acciones frm = new frmDescargoTransferencia_Acciones();
            frm.Text = "AGREGAR";
        ...
        private void frmCargoAjuste_Load(object sender, EventArgs e)
        {
            ...
            this.dgvFacturas.DataSource = NDescargoTransferencia.buscar_fact(cGeneral.id_user_actual, "", Convert.ToDateTime(dtpDesde.Value), Convert.ToDateTime(dtpHasta.Value));
            this.orden_fact(this.dgvFacturas);
            this.dgvProductos.DataSource = NDescargoTransferencia.cargar_prod(cGeneral.id_user_actual, "");

            this.txtRegistros.Text = NDescargoTransferencia.num_reg().ToString("N0");
        ...
        private void btnModificar_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt_datos = NDescargoTransferencia.obtener_datos(cGeneral.id_user_actual, Convert.ToInt32(this.dgvFacturas.CurrentRow.Cells[0].Value));
        ...
        private void tBuscar_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                this.dgvFacturas.DataSource = NDescargoTransferencia.buscar_fact(cGeneral.id_user_actual, this.txtBuscar.Text, Convert.ToDateTime(dtpDesde.Value), Convert.ToDateTime(dtpHasta.Value));
                this.orden_fact(this.dgvFacturas);
        ...
        private void btnAgregarProd_Click(object sender, EventArgs e)
        {
            if (this.dgvFacturas.Rows.Count > 0)
            {
                frmDescargoTransferencia_Productos frm = new frmDescargoTransferencia_Productos();
                frm.ShowDialog();
            }
            else
            {
                MessageBox.Show("No se puede realizar la acción de producto si no a ingresado el Descargo Transferencia", "Acción Producto", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        ...
        void click_eliminar_CJ()
        {
            try
            {
                DialogResult resul = MessageBox.Show("Está seguro(a) de eliminar éste Descargo Transferencia.", cGeneral.name_system, MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                ...
                NDescargoTransferencia.eliminar_CJ(Convert.ToInt32(this.dgvFacturas.CurrentRow.Cells[0].Value));
        ...
        private void tCargarProd_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.dgvFacturas.Rows.Count > 0)
                {
                    ...
                    this.dgvProductos.DataSource = NDescargoTransferencia.cargar_prod(cGeneral.id_user_actual, this.dgvFacturas.CurrentRow.Cells[0].Value.ToString());
                    cargar_totales(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString());

                    if (NDescargoTransferencia.verificar_cant_prod(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString()) == 1)
                    {
                    ...
                    else if (NDescargoTransferencia.verificar_cant_prod(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString()) == 2)
                    {
        ...
        void click_eliminar_prod()
        {
            if (this.dgvProductos.Rows.Count > 0)
            {
                try
                {
                    ...
                    NDescargoTransferencia.eliminar_prod(Convert.ToInt32(this.dgvFacturas.CurrentRow.Cells[0].Value), this.dgvProductos.CurrentRow.Cells["Id"].Value.ToString());
                    this.dgvProductos.Rows.Remove(this.dgvProductos.CurrentRow);

                    this.dgvFacturas.CurrentRow.Cells[2].Value = NDescargoTransferencia.actualizar_nombre_prov(cGeneral.id_user_actual, Convert.ToInt32(this.dgvFacturas.CurrentRow.Cells[0].Value));
                    DataTable dt_totales = NDescargoTransferencia.obtener_totales(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString());
        ...
        private void btnModificarProd_Click(object sender, EventArgs e)
        {
            ...
            if (this.dgvProductos.Rows.Count > 0)
            {
                frmDescargoTransferencia_Cantidad frm = new frmDescargoTransferencia_Cantidad();
        ...
        public void print_cargo_ajuste()
        {
            try
            {
                if (this.dgvProductos.Rows.Count > 0)
                {
                    DialogResult resul = MessageBox.Show("Desea imprimir el documento de descargo Transferencia de inventario.", cGeneral.name_system, MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    ...
                    Reportes.frmViewRpt frmReporte = new Reportes.frmViewRpt("descargo_transferencia_inv");
        ...
        void click_guardar()
        {
            try
            {
                ...
                NDescargoTransferencia.guardar(cGeneral.id_user_actual, Convert.ToInt32(this.dgvFacturas.CurrentRow.Cells[0].Value));
                this.dgvFacturas.CurrentRow.Cells[2].Value = NDescargoTransferencia.actualizar_nombre_prov(cGeneral.id_user_actual, Convert.ToInt32(this.dgvFacturas.CurrentRow.Cells[0].Value));
        ...
        {
            Reportes.frmViewRpt frmReporte = new Reportes.frmViewRpt("descargo_transferencia_inventario_rpt");
