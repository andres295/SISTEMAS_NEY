...
namespace SCM.CxP
{
    public partial class frmHistPagosProveedor : Form
    {
        public frmHistPagosProveedor()
        {
            InitializeComponent();
        }

        private void frmHistPagosCliente_Load(object sender, EventArgs e)
        {
        ...
        void click_buscar()
        {
            try
            {
                ...
                DataTable dt; 
                dt = NNotasCreditos.buscar_rpt_pagos(Convert.ToInt32(this.cmbProveedor.Value), Convert.ToDateTime(this.dtpDesde.Value), Convert.ToDateTime(this.dtpHasta.Value));
