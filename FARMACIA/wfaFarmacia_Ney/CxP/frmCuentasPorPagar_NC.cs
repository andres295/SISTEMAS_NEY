...
{
    public partial class frmCuentasPorPagar_NC : Form
    {
        public static frmCuentasPorPagar_NC me;
        public string nu_factura = "";
        public bool automatica = false;
        public frmCuentasPorPagar_NC()
        {
            frmCuentasPorPagar_NC.me = this;
            InitializeComponent();
        }
        ...
        private void frmCuentasPorPagar_NC_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.dgvFacturas.DataSource = NNotasCreditos.buscar("");
                this.orden_fact(this.dgvFacturas);
                this.dgvProductos.DataSource = NNotasCreditos.cargar_prod("");
                this.orden_prod(this.dgvProductos);

                this.txtRegistros.Text = NNotasCreditos.num_reg().ToString("N0");
        ...
        {
            frmCuentasPorPagar_NC_Agregar frm = new frmCuentasPorPagar_NC_Agregar();
            frm.Text = "AGREGAR";
            frm.automatica = this.automatica;
            frm.nu_factura = this.nu_factura;
            frm.ShowDialog();
        }
        ...
        private void btnModificar_Click(object sender, EventArgs e)
        {
            try
            {
                DataTable dt_datos = NNotasCreditos.obtener_datos_nc(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString());
                ...
                frm.txtProv.Text = dt_datos.Rows[0].ItemArray[10].ToString();
                ...
                frm.id_prov_captado = Convert.ToInt32(dt_datos.Rows[0].ItemArray[2].ToString());
                frm.prov_captado = dt_datos.Rows[0].ItemArray[10].ToString();
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (this.txtBuscar.Text == "")
                        dt_datos = NNotasCreditos.obtener_totales("");
                    else
                        dt_datos = NNotasCreditos.obtener_totales(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString());
        ...
        private void tBuscar_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                this.dgvFacturas.DataSource = NNotasCreditos.buscar(this.txtBuscar.Text);
                this.orden_fact(this.dgvFacturas);
                this.cargar_totales();
                ...
                {
                    this.dgvProductos.DataSource = NNotasCreditos.cargar_prod(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString());
                    this.orden_prod(this.dgvProductos);
        ...
        {
            frmCuentasPorPagar_NC_Productos frm = new frmCuentasPorPagar_NC_Productos();
            frm.ShowDialog();
        }
        ...
        private void dgvFacturas_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (this.dgvFacturas.Rows.Count > 0)
                {
                    this.dgvProductos.DataSource = NNotasCreditos.cargar_prod(this.dgvFacturas.Rows[e.RowIndex].Cells[0].Value.ToString());
                    this.orden_prod(this.dgvProductos);

                    if (NNotasCreditos.verificar(this.dgvFacturas.Rows[e.RowIndex].Cells[0].Value.ToString()) > 0)
                    {
                        this.btnAgregarProd.Enabled = false;
                        this.btnModificarProd.Enabled = false;
                        this.btnEliminarProd.Enabled = false;
                    ...
                    {
                        if (NNotasCreditos.verificar_temp(this.dgvFacturas.Rows[e.RowIndex].Cells[0].Value.ToString()) == 0)
                        {
                    ...
                    if (this.txtBuscar.Text == "")
                        ...
                        dt_datos = NNotasCreditos.obtener_totales(this.dgvFacturas.Rows[e.RowIndex].Cells[0].Value.ToString());
        ...
        {
            frmCuentasPorPagar_NC_Cantidad frm = new frmCuentasPorPagar_NC_Cantidad();
        ...
        void click_eliminar_prod()
        {
            try
            {
                ...
                NNotasCreditos.eliminar_prod(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString(), this.dgvProductos.CurrentRow.Cells[0].Value.ToString());
                this.dgvProductos.Rows.Remove(this.dgvProductos.CurrentRow);
                cargar_totales();

                this.dgvFacturas.CurrentRow.Cells[2].Value = NNotasCreditos.actualizar_nombre(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString());
        ...
        void click_guardar()
        {
            try
            {
                ...
                NNotasCreditos.guardar(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString(), this.dgvFacturas.CurrentRow.Cells[1].Value.ToString());
                frmCuentasPorPagar_NC.me.dgvFacturas.CurrentRow.Cells[2].Value = NNotasCreditos.actualizar_nombre(frmCuentasPorPagar_NC.me.dgvFacturas.CurrentRow.Cells[0].Value.ToString());

                for (int i = 0; i < this.dgvProductos.Rows.Count; i++)
                {
                    NNotasCreditos.actualizar_stock(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString(), this.dgvFacturas.CurrentRow.Cells[1].Value.ToString(), this.dgvProductos.Rows[i].Cells[0].Value.ToString(), true);
                }
                ...
                this.Close();
                ///frmCuentasPorPagar.me.actualizar_fila_fact(frmCuentasPorPagar.me.uGridFactura.Selected.Rows[0].Cells[0].Value.ToString());
            ...
            try
            {
                if (this.dgvFacturas.Rows.Count > 0)
                {
                    ...
                    if (this.dgvProductos.Rows.Count > 0)
                    {
                        for (int i = 0; i < this.dgvProductos.Rows.Count; i++)
                        {
                            NNotasCreditos.actualizar_stock(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString(), this.dgvFacturas.CurrentRow.Cells[1].Value.ToString(), this.dgvProductos.Rows[i].Cells[0].Value.ToString(), false);
                        }

                        NNotasCreditos.deducir_nc(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString(), this.dgvFacturas.CurrentRow.Cells[1].Value.ToString());
                    }

                    NNotasCreditos.eliminar_nc(this.dgvFacturas.CurrentRow.Cells[0].Value.ToString());
                ...
                else
                {
                    ...
                    this.ttMensaje.Show("No hay NC para eliminar", this.btnEliminar, 0, 26, 3000);
                }
            }
            catch (Exception ex) { cGeneral.error(ex); }
        }
